resource "fastly_service_v1" "app" {
  domain {
    name = "origami-image-service-dev.in.ft.com"
  }

  backend {
    name                  = "eu"
    address               = "origami-image-service-dev.herokuapp.com"
    port                  = 443
  ...
  healthcheck {
    name      = "eu_healthcheck"
    host      = "origami-image-service-dev.herokuapp.com"
    path      = "/__gtg"
    timeout   = 5000
    threshold = 2
    window    = 5
  ...
  header {
    ...
    source      = "\"origami-image-service-dev.herokuapp.com\""
  }
