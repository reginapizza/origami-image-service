...
function processImage(config) {
	return (request, response, next) => {
		...
		if ((transform.format === 'svg' || /\.svg/i.test(transform.uri)) && transform.tint) {
			...
			let baseUrl = `${request.protocol}://${hostname}/`;
			if (response.app.locals.baseUrl && response.app.locals.baseUrl !== '/') {
				baseUrl = response.app.locals.baseUrl;
			}
			transform.setUri(`${baseUrl}v2/images/svgtint/${encodedUri}?color=${tint}`);
		}
