...
function createProxy(errorHandler) {
	...
	function proxyResponseHandler(proxyResponse, request, response) {
		...
		proxyResponse.headers['Surrogate-Key'] = `${keyForAllImages} ${base64.encode(normaliseKeys(keyForImageType))} ${base64.encode(normaliseKeys(keyForScheme))} ${base64.encode(request.params.schemeUrl)}`;
